...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #buzzcoincoinTEST\r");
            Send(hSocket, "WHO #buzzcoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #buzzcoincoin\n");
            // randomly join #buzzcoincoin00-#buzzcoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #buzzcoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #buzzcoincoin%02d\r", channel_number).c_str());
        };
